...
func TestClientProperties(t *testing.T) {
	client := NewTelemetryClient(testIkey)
	...
	if ikey := client.InstrumentationKey(); ikey != testIkey {
		t.Error("Client's InstrumentationKey is not expected")
	}

	if ikey := client.Context().InstrumentationKey(); ikey != testIkey {
...
func TestEndToEnd(t *testing.T) {
	...
	config := NewTelemetryConfiguration(testIkey)
