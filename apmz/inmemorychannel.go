...
var (
	submitRetries = []time.Duration{time.Duration(10 * time.Second), time.Duration(30 * time.Second), time.Duration(60 * time.Second)}
)

// InMemoryChannel is a telemetry channel that stores events exclusively in memory.  Presently
...
// NewInMemoryChannel creates an InMemoryChannel instance and starts a background submission
// goroutine.
...
// Stop tears down the submission goroutines, closes internal channels.  Any
...
func (channel *InMemoryChannel) transmitRetry(items telemetryBufferItems, retry bool, retryTimeout time.Duration) {
	...
	for _, wait := range submitRetries {
