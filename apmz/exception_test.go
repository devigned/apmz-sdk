...
func TestExceptionTelemetry(t *testing.T) {
	...
	checkDataContract(t, "ExceptionDetails.TypeName", exd1.TypeName, "*apmz.myError")
	...
	checkDataContract(t, "ExceptionDetails.TypeName", exd2.TypeName, "*apmz.myStringer")
	...
	checkDataContract(t, "ExceptionDetails.TypeName", exd3.TypeName, "*apmz.myGoStringer")
}
...
func testExceptionCallstack(t *testing.T, n int) *contracts.ExceptionDetails {
	...
	for i := frstart; j >= 0 && i < len(ex.ParsedStack); i++ {
		...
		if !strings.HasSuffix(ex.ParsedStack[i].Assembly, "github.com/devigned/apmz-sdk/apmz") {
			checkDataContract(t, fmt.Sprintf("ParsedStack[%d].Assembly", i), ex.ParsedStack[i].Assembly, "github.com/devigned/apmz-sdk/apmz")
		}
