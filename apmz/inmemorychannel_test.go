...
const ten = time.Duration(10) * time.Second
...
func newTestChannelServer(config ...*TelemetryConfiguration) (TelemetryClient, *testTransmitter) {
	...
	if len(config) > 0 {
		...
		config.MaxBatchInterval = ten // assumed by every test.
...
func TestSimpleSubmit(t *testing.T) {
	...
	assertTimeApprox(t, req.timestamp, tm.Add(ten))

	if !strings.Contains(req.payload, "~msg~") {
...
func TestMultipleSubmit(t *testing.T) {
	...
	assertTimeApprox(t, req1.timestamp, start.Add(ten))
	...
	assertTimeApprox(t, req2.timestamp, start.Add(ten+ten))
...
func TestFlush(t *testing.T) {
	...
	req2 := transmitter.waitForRequest(t)
	assertTimeApprox(t, req2.timestamp, tm.Add(ten))
...
func TestCloseFlushRetry(t *testing.T) {
	...
	assertTimeApprox(t, req2.timestamp, tm.Add(submitRetries[0]))
}
...
func TestRetryOnFailure(t *testing.T) {
	...
	assertTimeApprox(t, req1.timestamp, tm.Add(ten))
	...
	assertTimeApprox(t, req2.timestamp, tm.Add(ten).Add(submitRetries[0]))
}
...
func TestThrottleStacking(t *testing.T) {
	...
	secondTm := transmitter.prepThrottle(time.Minute)
	...
	req3 := transmitter.waitForRequest(t)
	assertTimeApprox(t, req3.timestamp, secondTm)
	req4 := transmitter.waitForRequest(t)
	assertTimeApprox(t, req4.timestamp, secondTm)
